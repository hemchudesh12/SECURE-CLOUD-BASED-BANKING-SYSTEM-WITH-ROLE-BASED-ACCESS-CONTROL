-- ============================================================
-- FIX + SEED SCRIPT — SecureBank (Run in phpMyAdmin → banking_db → SQL)
-- ============================================================
USE banking_db;
...
ALTER TABLE accounts
  ...
  ADD COLUMN IF NOT EXISTS interest_rate DECIMAL(5,2) DEFAULT 3.50;

-- ── PATCH transaction_limits ────────────────────────────────
ALTER TABLE transaction_limits
  ADD COLUMN IF NOT EXISTS max_daily_count INT DEFAULT 10;

-- ── PATCH users ─────────────────────────────────────────────
ALTER TABLE users
  ADD COLUMN IF NOT EXISTS two_fa_enabled  TINYINT(1)  DEFAULT 0,
  ADD COLUMN IF NOT EXISTS two_fa_secret   VARCHAR(32),
  ADD COLUMN IF NOT EXISTS login_failures  INT         DEFAULT 0,
  ...
  ADD COLUMN IF NOT EXISTS remember_expiry DATETIME;
...
-- ── DEMO USERS ──────────────────────────────────────────────
-- Passwords: Hem@2806 | Teller@123 | Mani@123 | Customer@123
INSERT INTO users (id, username, full_name, email, phone, password_hash, role_id, is_active, email_verified)
VALUES
(1,'Hem',     'Hem (Administrator)','hem@bank.local',    '9000000001','$2y$12$zDYt2gSCw3o8Bm1kCGEXNOupF87St8qbFcNDuMFxpXrvnN8sGFGmO',1,1,1),
(2,'teller01','Priya Sharma',       'teller@bank.local', '9000000002','$2y$12$pqydmjZL1cydp8G0RTay8.CWewr5vfe0nz6HP3M1ssqlMV9Q.zITq', 2,1,1),
(3,'Mani',    'Mani (Customer)',    'mani@example.local','9000000003','$2y$12$TRT1e074.s1CmvNBjlL1z.TREZd/zXwMOuzIWX7Xt6Vm3oTWu9Pxa',3,1,1),
(4,'jane_doe','Jane Doe',           'jane@example.local','9000000004','$2y$12$lExNvJMD74tNmD3dPlWthOcxwF/8l8ec2DfjKuUhbudVz4toBQ/Zi', 3,1,1)
...
-- ── TRANSACTION LIMITS ──────────────────────────────────────
INSERT IGNORE INTO transaction_limits (role_id, daily_limit, single_limit, requires_approval_above, max_daily_count)
VALUES
...
-- ── BENEFICIARY ─────────────────────────────────────────────
INSERT IGNORE INTO beneficiaries (user_id, nickname, account_number, bank_name)
VALUES (3,'Jane Doe','ACC-20240002','SecureBank');

-- ── SAMPLE TRANSACTIONS ──────────────────────────────────────
INSERT IGNORE INTO transactions (reference_number, from_account_id, to_account_id, amount, type, status, initiated_by, description, created_at)
VALUES
...
-- ── NOTIFICATIONS ────────────────────────────────────────────
INSERT IGNORE INTO notifications (user_id,type,title,message,is_read,created_at)
VALUES
...
(1,'info',  'System Ready',   'All systems operational.',                          1,NOW());

-- ── SUPPORT TICKETS ──────────────────────────────────────────
INSERT IGNORE INTO support_tickets (user_id,subject,message,status,priority)
VALUES
(3,'Unable to make transfer','Transfer keeps failing. Please help.','open',       'high'),
(4,'Account statement query','Please send last 3 months statement.','in_progress','medium');

-- ── FRAUD FLAG ───────────────────────────────────────────────
INSERT IGNORE INTO fraud_flags (transaction_id, rule_triggered, risk_score, review_status)
SELECT id,'large_amount,high_frequency',65,'pending'
FROM transactions WHERE reference_number='TXN-20240105-MMNNOO' LIMIT 1;

SELECT 'SUCCESS: All fixes applied and seed data loaded!' AS result;
